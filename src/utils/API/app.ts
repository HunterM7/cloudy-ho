import { fetchData, getUrl } from './fetchData.ts'
import * as module from './constants.ts'
...
export const updateWeather = function (lat, lon) {
   ...
   */
  fetchData(getUrl.currentWeather(lat, lon), function (currentWeather) {
    const {
      weather,
      dt: dateUnix,
    ...
    fetchData(getUrl.reverseGeo(lat, lon), function ([{ name, country }]) {
     ...
     * TODAY'S HIGHLIGHTS
     */
    fetchData(getUrl.airPollution(lat, lon), function (airPollution) {
      const [
        {
          main: { aqi },
     ...
     */
    fetchData(getUrl.forecast(lat, lon), function (forecast) {
      const {
        list: forecastList,
        city: { timezone },
      } = forecast
